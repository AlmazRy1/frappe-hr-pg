...
def rename_fields():
    try:
        if frappe.db.has_column("Appraisal Template", "kra_title"):
            rename_field("Appraisal Template", "kra_title", "template_title")
        else:
            frappe.log_error("kra_title not found in Appraisal Template", "Rename Field Error")

        if frappe.db.has_column("Appraisal", "kra_template"):
            rename_field("Appraisal", "kra_template", "appraisal_template")
        else:
            frappe.log_error("kra_template not found in Appraisal", "Rename Field Error")
